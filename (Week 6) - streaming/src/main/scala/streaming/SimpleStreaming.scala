...
object SimpleStreaming extends ExtraStreamOps with SimpleStreamingInterface {
   ...
   */
  def filterUsingPreviousFilterFlowAndMapToStrings(ints: Source[Int, NotUsed]): Source[String, NotUsed] = mapToStrings(ints.via(filterEvenValues))
   ...
   */
  def firstElementFuture(ints: Source[Int, NotUsed])(implicit mat: Materializer): Future[Int] = ints.take(1).runWith(Sink.head)
  ...
  def recoverSingleElement(ints: Source[Int, NotUsed]): Source[Int, NotUsed] =
    ints.throttle(elements = 1, per = 1.milliseconds).recover {
      case ex : IllegalStateException => -1
    }
  ...
  def recoverToAlternateSource(ints: Source[Int, NotUsed], fallback: Source[Int, NotUsed]): Source[Int, NotUsed] =
    ints.throttle(elements = 1, per = 1.milliseconds).recoverWithRetries(attempts = 1, {
      case ex : IllegalStateException => {
        fallback.throttle(elements = 1, per = 1.milliseconds)
      }
    })
  ...
  def sumUntilBackpressureGoesAway: Flow[Int, Int, _] = {
    val p = 0.01
    Flow[Int].conflate[Int] {
      case (acc: Int, elem: Int) if Random.nextDouble < p => acc + elem
      case (acc: Int, _) => acc
    }
  }
