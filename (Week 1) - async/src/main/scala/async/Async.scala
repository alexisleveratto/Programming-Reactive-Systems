...
object Async extends AsyncInterface {
  ...
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] =
    eventuallyX map(evenNumber => evenNumber%2 == 0)
  ...
  def sequenceComputations[A, B](
    ...
    makeAsyncComputation1().flatMap { makeAsync1 =>
      makeAsyncComputation2().map(makeAsync2 => (makeAsync1, makeAsync2))
    }
  ...
  def concurrentComputations[A, B](
    ...
    val makeAsync1 = makeAsyncComputation1()
    val makeAsync2 = makeAsyncComputation2()
    makeAsync1.flatMap { async1 =>
      makeAsync2.map(async2 => (async1, async2))
    }
  }
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    makeAsyncComputation()
      .recoverWith {
        case _ if (maxAttempts != 0) => insist(makeAsyncComputation, maxAttempts - 1)
      }
  }
